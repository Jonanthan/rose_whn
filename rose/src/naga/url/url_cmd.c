      ...
      URL_EXPR)
{
    ...
    vty_out(vty, "%-6s %-16s %-64s %s %s", "ID", "cnt", "URL", "action", VTY_NEWLINE);
    for(i=0; i < MAX_URL_RULE; i++)
    {
		...
		if(pcreptr->used)
		{
        	...
        	vty_out(vty, "%-6d %-16ld %-64s %s %s", pcreptr->id, (uint64_t) pcreptr->acl.cnt.cnt, 
                    pcreptr->pattern, acl_str, VTY_NEWLINE);    
		}
